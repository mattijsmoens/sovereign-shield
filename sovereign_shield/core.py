...
class CoreSafety(metaclass=FrozenNamespace):
    ...
    def audit_action(cls, action_type, payload, invoker_role="Unknown",
        ...
        # Prevents action flooding. Minimum 0.5s between actions.
        with cls._LOCK:
            current_time = time.time()
            if (current_time - cls._STATE["last_action_time"]) < 0.5:
                return False, "Rate limited: minimum 0.5s between actions."
